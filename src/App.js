...
export default function App() {
  ...
  const User = ({ user }) => {
    ...
    return (
      <div>
        ...
        <button
          onClick={() =>
            setProfileData(user.password.length > 9 ? user : 'Profile is Fraud')
          }
        >
          Check Profile
        </button>
  ...
  return (
    <div>
      ...
      <h2>Profile Data</h2>
      {profileData !== null ? (
        profileData === 'Profile is Fraud' ? (
          <div>Profile is Fraud</div>
        ) : (
          <div>
            <h4>
            ...
            {profileData.subscription.status !== undefined && (
              <h4>{profileData.subscription.status}</h4>
            )}
            <h4>{profileData.subscription.plan}</h4>
          </div>
        )
      ) : (
        false
